...
%
   ...
   ListInNodes(1)= [];
    ...
    RumorPopularity2(1)=(1-pro)*10*20;
   % NbrInfected(count)=sum(NodeAccAtt(:)>0);
    max= RumorPopularity(count);
    ...
    Ne(count)= ((sum(NodeAccBelief(:)>=0)/Nodes)-S(count)-Q(count)-NN(count));
        ...
        TOtaleInfected=length(ListInNodes)
        Spreaders=NbrSpreader(count-1)
